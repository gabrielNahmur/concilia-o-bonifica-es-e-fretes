        ...
        return <section className="routine-section" key={group}>
          ...
          {rows.length ? <div className="routine-card-grid">{rows.map((card) => <article className={`routine-card routine-${card.situation}`} key={card.id}>
            ...
            {card.cumulative && <div className="routine-cumulative-note">
              ...
              {Number(card.historical_adjustment_value || 0) > 0.01 && <small>Ajuste histórico aprovado incluído em Créditos apropriados: {money(card.historical_adjustment_value)}. Créditos apropriados no portal: {money(card.portal_appropriated_value ?? (Number(card.observed_value || 0) - Number(card.historical_adjustment_value || 0)))}.</small>}
