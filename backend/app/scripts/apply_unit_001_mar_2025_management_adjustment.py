"""Classify the approved historical adjustment for unit 001 / March 2025.

The application already had an approved full adjustment for this competence.
This script adds a zero-value classification record so the operational screen
does not present a calculation memory as a distributor or ERP financial proof.
It changes only the Contracts application database.
"""
...
TARGET_EXPECTED = Decimal("10005.00")
ADJUSTMENT_REASON = (
    f"{FULL_MANAGEMENT_ADJUSTMENT_MARKER} Classificação do ajuste histórico integral "
    "já aprovado pela gestão: esta competência foi encerrada por decisão empresarial "
    "e não representa crédito emitido pela Ipiranga ou lançamento identificado no ERP."
)


def apply_unit_001_mar_2025_management_adjustment(
    ...
    if not row:
        raise ValueError("Conciliação 001 / março de 2025 não encontrada.")
    expected = money(Decimal(row.expected_value or 0))
    current_adjustment = money(Decimal(row.manual_adjustment or 0))
    observed = money(Decimal(row.observed_value or 0))
    if expected != TARGET_EXPECTED:
        raise ValueError(f"Valor esperado inesperado: R$ {expected:.2f}.")
    if current_adjustment != expected or observed != Decimal("0.00"):
        raise ValueError(
            "A competência não representa um ajuste integral sem crédito financeiro identificado."
        )
    existing = db.scalar(
...
def main() -> None:
    with SessionLocal() as db:
        result = apply_unit_001_mar_2025_management_adjustment(db)
        audit(
            db,
            None,
            "unit_001_mar_2025_full_management_adjustment",
            "reconciliation",
            "001:2025-03",
